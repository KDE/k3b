...
void K3bDvdRippingProcess::preProcessingDvd( ) {
    ...
    if( !m_dvdOrgFilenameDetected ) {
        K3bDevice::Device *dev = k3bcore->deviceManager()->deviceByName( m_device );
...
void K3bDvdRippingProcess::saveConfig() {
    ...
    if( f.exists() ) {
        ...
        int button = KMessageBox::warningContinueCancel(qApp->activeWindow(), i18n("Log file already exists. Overwrite?"), i18n("Ripping Error"),
        i18n("Overwrite"), dontAskAgainName) ;
