...
void K3bCdrdaoWriter::slotProcessExited( KProcess* p ) {
  ...
  case COPY:
	...
	kdDebug() << "(K3bCdrdaoWriter) restoring tocfile " 
		  << m_tocFile << " failed." << endl;
    ...
    if ( !KIO::NetAccess::del(m_backupTocFile) )
      kdDebug() << "(K3bCdrdaoWriter) delete tocfile backkup " 
		<< m_backupTocFile << " failed." << endl;
    break;
  case BLANK:
  case READ:
    break;
