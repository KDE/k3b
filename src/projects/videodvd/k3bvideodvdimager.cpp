...
int K3bVideoDvdImager::writePathSpec()
{
  ...
  for( QPtrListIterator<K3bDataItem> it( d->doc->videoTsDir()->children() ); *it; ++it ) {
    if( (*it)->isDir() ) {
      kdDebug() << "(K3bVideoDvdImager) no directories allowed in the VIDEO_TS dir." << endl;
      return -1;
    }
    ...
    if( ::symlink( QFile::encodeName( (*it)->localPath() ), 
      ...
      kdDebug() << "Unable to link temporary file in folder " << d->tempPath << endl;
      return -1;
    }
  }
  ...
  if( QTextStream* t = m_pathSpecFile->textStream() ) {
    ...
    //
    int num = d->doc->videoTsDir()->children().count();
    d->doc->root()->takeDataItem( d->doc->videoTsDir() );

    num += writePathSpecForDir( d->doc->root(), *t );
    d->doc->root()->addDataItem( d->doc->videoTsDir() );
