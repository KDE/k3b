...
void K3bDataFileView::checkForNewItems()
{
    ...
    Q_FOREACH( K3bDataItem* item, m_currentDir->children() ) {
        if( !m_itemMap.contains( item ) ) {
            slotItemAdded( item );
        }
    }
...
{
    QList<Q3ListViewItem*> selectedViewItems = selectedItems();
    KUrl::List urls;
...
void K3bDataFileView::slotRemoveItem()
{
    ...
    Q_FOREACH( Q3ListViewItem* item, items ) {
        if( K3bDataViewItem* d = dynamic_cast<K3bDataViewItem*>( item ) )
...
void K3bDataFileView::slotProperties()
{
    ...
    // get selected item
    QList<Q3ListViewItem*> viewItems = selectedItems();
    Q_FOREACH( Q3ListViewItem* item, viewItems ) {
        if( K3bDataViewItem* viewItem = dynamic_cast<K3bDataViewItem*>( item ) ) {
    ...
    if( K3bDataViewItem* viewItem = dynamic_cast<K3bDataViewItem*>( selectedItems().first() ) ) {
        ...
        if( item->isFile() ) {
            ...
            if( fvi->mimeType() &&
                ...
                KUrl::List urls;
                urls.append( KUrl(item->localPath()) );
                KRun::displayOpenWithDialog( urls, false, this );
            }
        }
    }
}
