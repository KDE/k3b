		 ...
		 true )
{
  ...
  QLabel* label = new QLabel( i18n("Please select the appendable disk"), plainPage() );
...
void K3bDataSessionImportDialog::importSession( K3bDataDoc* doc )
{
  ...
  m_processLabel->setText( i18n("Please Wait...") );
...
void K3bDataSessionImportDialog::checkNextDevice()
{
  ...
  if( dev && !m_alreadySelected ) {
    ...
    if( m_selectionBox->count() == 0 ) {
      ...
      m_processLabel->setText( i18n("Could not find a session to import.") );
    }
    else {
      showButton( KDialogBase::Ok, true );
      showButton( KDialogBase::Cancel, true );

      m_processLabel->setText( i18n("Please select the appendable disk") );
    }
  }
}
