...
void KioTreeTopLevelItem::setOpen( bool open )
{
    if (open && module())
...
// !! for now we don't provide actions based on the browserExtension !!
void KioTreeTopLevelItem::itemSelected()
{
//     kdDebug() << "KioTreeTopLevelItem::itemSelected" << endl;
...
// bool KioTreeTopLevelItem::acceptsDrops( const QStrList & formats )
// {
//     return formats.contains("text/uri-list") &&
//         ( m_bTopLevelGroup || !externalURL().isEmpty() );
// }

// void KioTreeTopLevelItem::drop( QDropEvent * ev )
// {
//     if ( m_bTopLevelGroup )
//     {
...
QDragObject * KioTreeTopLevelItem::dragObject( QWidget * parent, bool move )
{
    // 100% duplicated from KonqDirTreeItem::dragObject :(
    KURL::List lst;
    KURL url;
    url.setPath( path().isEmpty() ? externalURL().path() : path() );
    lst.append( url );
...
void KioTreeTopLevelItem::middleButtonPressed()
{
//     if ( !m_bTopLevelGroup )
...
void KioTreeTopLevelItem::rightButtonPressed()
{
  // !! for now we don't use browserExtensions !! But this should be added to all fileviews (doesn't it??)
//     KURL url;
//     url.setPath( m_path );
...
// void KioTreeTopLevelItem::trash()
// {
//     delOperation( KonqOperations::TRASH );
// }

// void KioTreeTopLevelItem::del()
// {
//     delOperation( KonqOperations::DEL );
// }

// void KioTreeTopLevelItem::shred()
// {
//     delOperation( KonqOperations::SHRED );
// }

// void KioTreeTopLevelItem::delOperation( int method )
// {
//     KURL url;
//     url.setPath( m_path );
//     KURL::List lst;
...
// void KioTreeTopLevelItem::paste()
// {
//     // move or not move ?
//     bool move = false;
//     QMimeSource *data = QApplication::clipboard()->data();
...
// void KioTreeTopLevelItem::rename()
// {
//     tree()->rename( this, 0 );
// }

// !! only works in extended mode !!
// void KioTreeTopLevelItem::rename( const QString & name )
// {
//     KURL url;
//     url.setPath( m_path );
...
QString KioTreeTopLevelItem::toolTipText() const
{
    return m_comment;
}
