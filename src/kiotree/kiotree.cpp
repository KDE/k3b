...
#include "kiotreedirmodule.h" // TEMPORARY HACK
...
KioTree::KioTree( /*KioTreePart *parent,*/ QWidget *parentWidget )
...
//     assert( KioTreeFactory::instance()->dirs() );
//     QString dirtreeDir = KioTreeFactory::instance()->dirs()->saveLocation( "data", "konqueror/dirtree/" );
//     m_dirtreeDir.setPath( dirtreeDir );

// TODO: fix extended mode, we don't need KDirNotify for simple mode since it is only used to check changes in 
//       m_dirtreeDir !!

    // Initial parsing in extended mode and some defaults in simple mode
    rescanConfiguration();
}
...
void KioTree::followURL( const KURL &url )
{
    // Maybe we're there already ?
    KioTreeItem *selection = static_cast<KioTreeItem *>( selectedItem() );
    ...
    QListIterator<KioTreeTopLevelItem> topItem ( m_topLevelItems );
...
void KioTree::contentsDragEnterEvent( QDragEnterEvent *ev )
{
    m_dropItem = 0;
...
void KioTree::contentsDragMoveEvent( QDragMoveEvent *e )
{
    ...
    if ( !item || !item->isSelectable() || !static_cast<KioTreeItem*>(item)->acceptsDrops( m_lstDropFormats ))
    {
        m_dropItem = 0;
...
void KioTree::contentsDragLeaveEvent( QDragLeaveEvent * )
{
...
void KioTree::contentsDropEvent( QDropEvent *ev )
{
    m_autoOpenTimer->stop();

    // ------------- dropping disabled for now!
...
void KioTree::contentsMousePressEvent( QMouseEvent *e )
{
...
void KioTree::contentsMouseMoveEvent( QMouseEvent *e )
{
    ...
    // Start a drag
    QDragObject * drag = static_cast<KioTreeItem *>( item )->dragObject( viewport(), false );
    if ( !drag )
	return;
...
void KioTree::contentsMouseReleaseEvent( QMouseEvent *e )
{
    KListView::contentsMouseReleaseEvent( e );
    m_bDrag = false;
}

void KioTree::leaveEvent( QEvent *e )
{
    KListView::leaveEvent( e );
    //    m_part->emitStatusBarText( QString::null );

    // TODO: add a signal that gives the url to anyone who wants it
}


void KioTree::slotDoubleClicked( QListViewItem *item )
{
    //kdDebug(1201) << "KioTree::slotDoubleClicked " << item << endl;
    if ( !item )
        return;

    if ( !static_cast<KioTreeItem*>(item)->isClickable() )
        return;
...
void KioTree::slotClicked( QListViewItem *item )
{
    //kdDebug(1201) << "KioTree::slotClicked " << item << endl;
    if ( !item )
        return;
    ...
    KioTreeItem *dItem = static_cast<KioTreeItem *>( item );

    emit urlActivated( dItem->externalURL() );
...
void KioTree::slotMouseButtonPressed(int _button, QListViewItem* _item, const QPoint&, int col)
{
    KioTreeItem * item = static_cast<KioTreeItem*>(_item);
    if(_item && col < 2)
...
void KioTree::slotAutoOpenFolder()
{
    m_autoOpenTimer->stop();
...
void KioTree::rescanConfiguration()
{
    kdDebug(1201) << "KioTree::rescanConfiguration()" << endl;
    m_autoOpenTimer->stop();
    clearTree();
    //    scanDir( 0, m_dirtreeDir.path(), true);  // simple mode
}

void KioTree::slotSelectionChanged()
{
    if ( !m_dropItem ) // don't do this while the dragmove thing
    {
        KioTreeItem * item = static_cast<KioTreeItem *>( selectedItem() );
        if ( item )
            item->itemSelected();
...
// !! only important for extended mode !!
// void KioTree::FilesAdded( const KURL & dir )
// {
//     kdDebug(1201) << "KioTree::FilesAdded " << dir.url() << endl;
...
// !! only important for extended mode !!
// void KioTree::FilesRemoved( const KURL::List & urls )
// {
//     //kdDebug(1201) << "KioTree::FilesRemoved " << urls.count() << endl;
//     for ( KURL::List::ConstIterator it = urls.begin() ; it != urls.end() ; ++it )
//     {
//         //kdDebug(1201) <<  "KioTree::FilesRemoved " << (*it).prettyURL() << endl;
//         if ( m_dirtreeDir.isParentOf( *it ) )
//         {
//             QTimer::singleShot( 0, this, SLOT( rescanConfiguration() ) );
//             kdDebug(1201) << "KioTree::FilesRemoved done" << endl;
//             return;
//         }
//     }
// }
...
// !! only important for extended mode !!
// void KioTree::FilesChanged( const KURL::List & urls )
// {
//     //kdDebug(1201) << "KioTree::FilesChanged" << endl;
//     // not same signal, but same implementation
//     FilesRemoved( urls );
// }


// !! only importtant for extended mode !!
// void KioTree::scanDir( KioTreeItem *parent, const QString &path, bool isRoot )
// {
//     QDir dir( path );
...
//             kdDebug(1201) << "KioTree::scanDir found version " << versionNumber << endl;
...
//             QString dirtree_dir = KioTreeFactory::instance()->dirs()->findDirs( "data", "konqueror/dirtree" ).last(); // most global
//             kdDebug(1201) << "KioTree::scanDir dirtree_dir=" << dirtree_dir << endl;

//             /*
//             // debug code
//             QStringList blah = KioTreeFactory::instance()->dirs()->findDirs( "data", "konqueror/dirtree" );
//             QStringList::ConstIterator eIt = blah.begin();
//             QStringList::ConstIterator eEnd = blah.end();
//             for (; eIt != eEnd; ++eIt )
//                 kdDebug(1201) << "KioTree::scanDir findDirs got me " << *eIt << endl;
//             // end debug code
//             */
...
// !! only important for extended mode !!
// void KioTree::loadTopLevelGroup( KioTreeItem *parent, const QString &path )
// {
//     QDir dir( path );
...
//     KioTreeTopLevelItem *item;
//     if ( parent )
//     {
//         kdDebug(1201) << "KioTree::loadTopLevelGroup Inserting new group under parent " << endl;
//         item = new KioTreeTopLevelItem( parent, 0 /* no module */, path );
//     }
//     else
//         item = new KioTreeTopLevelItem( this, 0 /* no module */, path );
//     item->setText( 0, name );
...
// !! only important for extended mode !!
// void KioTree::loadTopLevelItem( KioTreeItem *parent,  const QString &filename )
// {
...
//     KioTreeTopLevelItem *item;
//     if ( parent )
//         item = new KioTreeTopLevelItem( parent, module, path );
//     else
//         item = new KioTreeTopLevelItem( this, module, path );
...
void KioTree::addTopLevelDir( const KURL& url, const QString& name )
{
  KioTreeDirModule* module = new KioTreeDirModule( this );

  KioTreeTopLevelItem *item = new KioTreeTopLevelItem( this, module, QString::null );

  // the topLevelItem does not have a .desktop file, so the Tree sets the external url 
  // instead of the module
  item->setExternalURL( url );
  
  item->setText( 0, name );
  item->setPixmap( 0, SmallIcon( KMimeType::iconForURL(url) ));
...
void KioTree::slotAnimation()
{
    ...
    for (; it != end; ++it )
    {
        ...
        it.key()->setPixmap( 0, SmallIcon( icon/*, KioTreeFactory::instance()*/ ) );
...
void KioTree::startAnimation( KioTreeItem * item, const char * iconBaseName, uint iconCount )
{
...
void KioTree::stopAnimation( KioTreeItem * item )
{
...
KioTreeItem * KioTree::currentItem() const
{
    return static_cast<KioTreeItem *>( selectedItem() );
}

void KioTree::slotOnItem( QListViewItem *item )
{
//     KioTreeItem *i = static_cast<KioTreeItem *>( item );
...
void KioTree::setContentsPos( int x, int y )
{
    if ( !m_scrollingLocked )
	KListView::setContentsPos( x, y );
}

void KioTree::slotItemRenamed(QListViewItem* item, const QString &name, int col)
{
//     ASSERT(col==0);
//     if (col != 0) return;
//     assert(item);
//     KioTreeItem * treeItem = static_cast<KioTreeItem *>(item);
//     if ( treeItem->isTopLevelItem() )
//     {
//         KioTreeTopLevelItem * topLevelItem = static_cast<KioTreeTopLevelItem *>(treeItem);
...
void KioTreeToolTip::maybeTip( const QPoint &point )
{
    QListViewItem *item = m_view->itemAt( point );
    if ( item ) {
	QString text = static_cast<KioTreeItem*>( item )->toolTipText();
	if ( !text.isEmpty() )
