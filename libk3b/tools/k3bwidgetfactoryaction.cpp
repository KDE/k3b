...
QWidget* K3bWidgetFactoryAction::widget( QWidget* container )
{
    ...
    if ( index != -1 ) {
        if ( ::qt_cast<KToolBar*>( container ) ) {
            ...
            kDebug() << "(K3bWidgetFactoryAction) container is no KToolBar: " << container;
        }
    }
    else {
        kDebug() << "(K3bWidgetFactoryAction) could not find container " << container;
    }
...
int K3bWidgetFactoryAction::plug( QWidget* container, int index )
{
    ...
    if ( ::qt_cast<KToolBar *>( container ) ) {
        ...
        kDebug() << "(K3bWidgetFactoryAction) added widget " << widget << " for container " << container << " with id " << id;
